...
任务存储在 `~/.openclaw/cron/jobs.json` 中，包含以下字段：

- **jobId**：任务唯一标识
- **schedule**：调度方式（at/every/cron）
- **payload**：要执行的内容
- **delivery**：结果发送位置（可选）
- **isolated**：是否在独立会话运行（推荐 true）

## 5. 管理技巧

**查看任务执行历史**：每个独立任务会创建新的会话 ID，可以在对话历史中查看。

**禁用 Cron**：如果不需要定时任务功能，可以在配置中设置 `cron.enabled: false`。

**错误恢复**：如果任务长时间未执行，系统会自动触发重新运行。
