...
OpenClaw 的架构分为四层。最上层是消息渠道（Channels），支持 Telegram、Discord、Slack、CLI 等多种接入方式，你可以在任何地方给它发指令。第二层是智能决策核心（Brain），负责 LLM 推理、任务分解与规划、工具选择与调用。第三层是技能插件系统（Skills），提供文件操作、Shell 命令、浏览器控制、API 集成等能力。最底层是双模记忆系统（Memory），包括 SOUL.md（身份与行为指令）、MEMORY.md（长期记忆）和对话历史（短期上下文）。

这种分层设计让 OpenClaw 既灵活又可控。你可以通过修改 SOUL.md 来定制 Agent 的性格和行为准则，通过 MEMORY.md 让它记住你的偏好和项目信息，通过安装不同的技能来扩展它的能力。
...
第三阶段"构建 Claw"（4-8 周）是深入理解原理，从零实现一个简化版的 AI Agent。你会学习 Agent Loop 和工具调用的核心机制，技能系统的解析和加载，多渠道消息适配器的设计，以及 SOUL.md 和 MEMORY.md 的记忆系统。
